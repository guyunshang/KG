...
class EntityMerger:
    ...
    def _enrich_candidates_with_type(self, candidates_batch: List[List[Any]]) -> List[List[str]]:
        """
        为候选实体列表补充类型信息，并处理非标准的数据格式。
        解决 unhashable type: 'list' 问题的核心方法。

        Args:
            candidates_batch: 候选实体列表批次

        Returns:
            List[List[str]]: 格式化后的实体列表 ["实体名 [类型]"]
        """
        # 1. 提取纯文本ID（数据清洗）
        clean_batch = []
        all_entity_ids = set()

        for group in candidates_batch:
            clean_group = []
            for entity in group:
                entity_id = None
                # 兼容多种数据格式，防止将列表或字典直接放入集合导致报错
                if isinstance(entity, str):
                    ...
                    # 处理 {'id': '实体名'} 格式
                    entity_id = entity['id']
                elif isinstance(entity, tuple) and len(entity) > 0:
                    entity_id = str(entity[0])

                if entity_id:
                    clean_group.append(entity_id)
                    all_entity_ids.add(entity_id)

            if len(clean_group) > 1:
                clean_batch.append(clean_group)
        ...
        """
        try:
            result = self.graph.query(query, params={"ids": list(all_entity_ids)})
            type_map = {row['id']: row.get('type', '未知') for row in result}
        except Exception as e:
            print(f"查询实体类型失败: {e}")
            type_map = {}
        ...
        # 3. 拼接类型信息
        enriched_batch = []
        for group in clean_batch:
            enriched_group = []
            for entity_id in group:
                etype = type_map.get(entity_id, '未知')
                enriched_group.append(f"{entity_id} [{etype}]")
            enriched_batch.append(enriched_group)
    ...
    def get_merge_suggestions(self, duplicate_candidates: List[Any]) -> List[List[str]]:
        ...
        for batch_start in range(0, candidate_count, optimal_batch_size):
            ...
            # === 修改点：调用数据增强方法，添加类型信息并清洗数据 ===
            # 这解决了跨类型合并错误和列表不可哈希的错误
            enriched_batch = self._enrich_candidates_with_type(batch)
    ...
    def process_duplicates(self, duplicate_candidates: List[Any]) -> Tuple[int, Dict[str, Any]]:
        ...
        # 过滤处理数量过少的候选组
        filtered_candidates = fixed_candidates

...
class GraphWriter:
    ...
    def __init__(self, graph: Neo4jGraph = None, batch_size: int = 50, max_workers: int = 2):
        """
        初始化图写入器
    ...
    def convert_to_graph_document(self, chunk_id: str, input_text: str, result: str) -> GraphDocument:
        """
        将提取的实体关系文本转换为GraphDocument对象
        *** 完整增强版：包含调试信息、宽松正则、Neo4j显示优化 ***
        """
        # --- 1. 定义宽松正则 (兼容中文标点、换行、空格) ---
            ...
            for match in rel_matches:
                ...
                if source_id not in nodes:
                    if source_id in self.node_cache:
                        ...
                        new_node = Node(
                            id=source_id,
                            type="未知",  # 或者使用 "Unknown"
                            # === 关键修改：这里也要加 id ===
                            properties={
                                'id': source_id,
                                'description': '自动补充的关联节点'
                            }
                        )
                        nodes[source_id] = new_node
                        self.node_cache[source_id] = new_node
        ...
        #     print(f"[DEBUG] ❌ Chunk {chunk_id} 未提取到有效内容")

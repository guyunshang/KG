...
from graphrag_agent.config.settings import entity_types, relationship_types, entity_definitions, relationship_definitions # 确保引入 entity_definitions
...
class EntityRelationExtractor:
    ...
    def __init__(self, llm, system_template, human_template,
             entity_types: List[str], relationship_types: List[str], entity_definitions: dict = None,
             relationship_definitions: dict = None,cache_dir="./cache/graph", max_workers=4, batch_size=5):
        """
        初始化实体关系提取器
        ...
        e_defs = entity_definitions or {}
        r_defs = relationship_definitions or {}

        self.formatted_entity_types = "\n".join([
            f"- {etype}: {e_defs.get(etype, '无详细定义')}"
            for etype in self.entity_types
        ])

        self.formatted_relationship_types = "\n".join([
            f"- {rtype}: {r_defs.get(rtype, '无详细定义')}"
    ...
    def process_chunks_batch(self, file_contents: List[Tuple], progress_callback=None) -> List[Tuple]:
        ...
        """
        try:
            # 尝试局部导入以防全局导入失效（虽然上面已经修正了全局导入）
            from graphrag_agent.config.settings import entity_definitions as ed, relationship_definitions as rd
            current_entity_defs = ed
            current_rel_defs = rd
        except ImportError:
            current_entity_defs = {}
            current_rel_defs = {}
        ...
        formatted_e_types = "\n".join([
            f"- {etype}: {current_entity_defs.get(etype, '无详细定义')}"
            for etype in self.entity_types
        ])
        formatted_r_types = "\n".join([
            f"- {rtype}: {current_rel_defs.get(rtype, '无详细定义')}"
            for rtype in self.relationship_types
        ])
    ...
    def _process_single_chunk(self, input_text: str) -> str:
        ...
        response = self.chain.invoke({
            "chat_history": self.chat_history,
            "entity_types": self.formatted_entity_types,
            "relationship_types": self.formatted_relationship_types,
            "tuple_delimiter": self.tuple_delimiter,
            "record_delimiter": self.record_delimiter,
            "completion_delimiter": self.completion_delimiter,

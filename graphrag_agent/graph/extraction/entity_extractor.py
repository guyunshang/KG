...
from graphrag_agent.config.settings import entity_types, relationship_types, entity_definitions # 确保引入 entity_definitions
...
class EntityRelationExtractor:
    ...
    def _process_single_chunk(self, input_text: str) -> str:
        ...
        self.formatted_entity_types = "\n".join([
            f"- {etype}: {entity_definitions.get(etype, '无详细定义')}"
            for etype in self.entity_types
        ])

        self.formatted_relationship_types = "\n".join([
            f"- {rtype}: {relationship_definitions.get(rtype, '无详细定义')}"

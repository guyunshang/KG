...
from graphrag_agent.pipelines.ingestion.pdf_processor import pdf_to_text
...
class FileReader:
    ...
    def _read_pdf(self, file_path: str) -> str:
        ...
        try:
            ...
            print(f"使用新方法读取PDF失败，回退到原方法: {str(e)}")
            text = ""
            try:
                with pdfplumber.open(file_path) as pdf:
                    for page_num, page in enumerate(pdf.pages, start=1):
                        try:
                            ...
                            print(f"读取PDF文件 {os.path.basename(file_path)} 第 {page_num} 页失败: {str(e)}")
                            text += f"[第 {page_num} 页无法读取]\n\n"
            except Exception as e:
                print(f"读取PDF文件 {os.path.basename(file_path)} 失败: {str(e)}")
                text = f"[无法读取PDF文件内容: {str(e)}]"
            return text
    ...
    def _read_docx(self, file_path: str) -> str:
        """读取Word文档(.docx)，包括表格内容"""
        try:
            doc = Document(file_path)
            full_text = []

            # 遍历文档中的所有元素，保持段落和表格的原始顺序
                ...
                # 处理表格
                    ...
                    for row in doc.tables[table_idx].rows:
                        ...
                        for cell in row.cells:
                            ...
                            row_text += f"{cell_text}  "  # 使用两个空格分隔单元格
                        # 确保不会去除末尾的空格
                        table_text += row_text + "\n"
    ...
    def _read_doc(self, file_path: str) -> str:
        ...
        try:
            ...
            try:
                ...
                print(f"使用win32com读取.doc文件时出错: {e}")
                return ""
